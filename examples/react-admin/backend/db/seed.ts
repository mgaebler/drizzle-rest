...
Then configure your database connection and schema...`,
                    excerpt:
                        "Learn how to set up and use Drizzle ORM in your TypeScript projects with this comprehensive guide.",
...
Let's walk through each step...`,
                    excerpt:
                        "A complete guide to building type-safe REST APIs using Express.js and TypeScript.",
...
- Consider query result caching`,
                    excerpt:
                        "Essential tips and strategies for optimizing database performance in production applications.",
...
- Consider code splitting with React.lazy`,
                    excerpt:
                        "Explore modern React development patterns and best practices for building scalable applications.",
...
- Set proper health checks`,
                    excerpt:
                        "Learn how to containerize your Node.js applications effectively using Docker.",
        ...
        await db.insert(schema.postTags).values([
            ...
            {
                postId: posts[0].id,
                tagId: tags.find((t) => t.slug === "postgresql")!.id,
            },
            {
                postId: posts[1].id,
                tagId: tags.find((t) => t.slug === "typescript")!.id,
            ...
            {
                postId: posts[1].id,
                tagId: tags.find((t) => t.slug === "express")!.id,
            },
            {
                postId: posts[1].id,
                tagId: tags.find((t) => t.slug === "api")!.id,
            },
            {
                postId: posts[1].id,
                tagId: tags.find((t) => t.slug === "rest")!.id,
            },
            {
                postId: posts[2].id,
                tagId: tags.find((t) => t.slug === "postgresql")!.id,
            ...
            {
                postId: posts[2].id,
                tagId: tags.find((t) => t.slug === "performance")!.id,
            },
            {
                postId: posts[3].id,
                tagId: tags.find((t) => t.slug === "react")!.id,
            },
            {
                postId: posts[3].id,
                tagId: tags.find((t) => t.slug === "typescript")!.id,
            ...
            {
                postId: posts[4].id,
                tagId: tags.find((t) => t.slug === "docker")!.id,
            },
        ]);
        ...
        const commentsResult = await db
            ...
            .values([
                {
                    content:
                        "Great introduction to Drizzle! I've been looking for a TypeScript-first ORM.",
                    postId: posts[0].id,
                    userId: users[1].id,
                },
                {
                    content:
                        "Thanks for the detailed setup guide. The type safety features are impressive.",
                    postId: posts[0].id,
                    userId: users[2].id,
                },
                {
                    content:
                        "This is exactly what I needed for my Express API project. Thanks!",
                    postId: posts[1].id,
                    userId: users[0].id,
                },
                {
                    content:
                        "The indexing strategies section was particularly helpful. Any thoughts on partial indexes?",
                    postId: posts[2].id,
                    userId: users[3].id,
                },
                {
                    content:
                        "Docker has been a game-changer for our deployment process. Great writeup!",
                    postId: posts[4].id,
                    userId: users[0].id,
                },
            ])
        ...
        const commentsArray = Array.isArray(commentsResult)
            ? commentsResult
            : [commentsResult];
        ...
        if (commentsArray.length > 3) {
            await db.insert(schema.comments).values([
                {
                    content:
                        "Partial indexes are great for conditional data! I should cover that in a follow-up post.",
                    postId: posts[2].id,
                    userId: users[1].id, // Original author replying
